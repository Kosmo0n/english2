...
const listeningData = {
  ...
  questions: [
    ...
    {
      id: 'q2',
      question: 'What flight number is mentioned?',
      options: ['BA402', 'BA024', 'BA204', 'AB204'],
      answer: 2,
    },
    {
      id: 'q3',
      question: 'What does the speaker need to do before security?',
      options: ['Buy a ticket', 'Exchange currency', 'Drop luggage', 'Show passport'],
      answer: 2,
    },
  ],
}
...
export default function ListeningStep({ onComplete }: ListeningStepProps) {
  ...
  const score = submitted
    ? answers.filter((a, i) => a === listeningData.questions[i].answer).length
    : 0
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-white/50">{listeningData.title}</p>
      </div>
      ...
      {played && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          {submitted && (
            ...
            >
              <div className="text-3xl mb-1">{score === 3 ? '🎉' : score >= 2 ? '👍' : '📚'}</div>
              <div className="font-bold text-lg gradient-text">{score}/3 correct</div>
