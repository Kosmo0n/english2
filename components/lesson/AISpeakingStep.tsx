...
export default function AISpeakingStep({ onComplete }: { onComplete: () => void }) {
  ...
  const startRecording = () => {
    ...
    if (SR) {
      ...
      r.onresult = (e: any) => {
        const current = Array.from(e.results).map((result: any) => result[0].transcript).join('')
      ...
      r.onerror = (e: any) => {
        ...
        if (e.error === 'not-allowed') {
          alert('Microphone access is blocked. Please allow microphone access in your browser settings to continue.');
          setState('idle');
      ...
      // Fallback
      setTimeout(() => {
        setTranscript('I went to London last year and visited many historical sites.')
        processRecording()
      }, 3000)
    }
  }
  ...
  const processRecording = async () => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          systemPrompt: 'You are an English language expert. Evaluate the following student response based on the prompt. Provide a score from 1 to 100 and a short constructive feedback comment. Format your response exactly like this: SCORE: [number] | FEEDBACK: [comment]',
          messages: [{ role: 'user', content: `Prompt: ${prompt.text} | Student Response: ${transcript}` }]
        })
      });
      ...
      const comment = feedbackMatch ? feedbackMatch[1].trim() : "Good effort! Keep practicing your pronunciation.";
      
      setFeedback({ score, comment });
      setState('done');
    } catch (error) {
      console.error('Failed to process speaking feedback:', error);
      setFeedback({ score: 75, comment: "Connection issues. But your speech was captured!" });
      setState('done');
    }
  }
