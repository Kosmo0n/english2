...
const defaultQuestions: Question[] = [
  {
    id: 'f1',
    question: 'What is the main goal of fluency?',
    options: ['Perfect grammar', 'Fast speaking', 'Easy communication', 'Large vocabulary'],
    answer: 2,
    explanation: 'Fluency is primarily about being able to communicate easily and accurately.',
  },
]

export default function FinalTestStep({ onComplete, questions = defaultQuestions }: FinalTestStepProps) {
  const [answers, setAnswers] = useState<(number | null)[]>(Array(questions?.length || 1).fill(null))
  ...
  const score = submitted
    ? answers.filter((a, i) => a === currentQuestions[i].answer).length
    : 0
      ...
      <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6 text-center">
        ...
        <div className="glass rounded-2xl p-6 border border-white/10">
          <div className="grid grid-cols-3 gap-4 mb-5">
            {[
              { label: 'Score', value: `${score}/${currentQuestions.length}` },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/50">Answer all {currentQuestions.length} questions</p>
        </div>
        <div className="text-sm text-white/50">{answers.filter(a => a !== null).length}/{currentQuestions.length} answered</div>
      </div>
