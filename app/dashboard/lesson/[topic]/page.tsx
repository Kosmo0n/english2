...
import { lessonData, defaultLessonContent } from '@/lib/data'
...
export default function LessonPage({ params }: { params: { topic: string } }) {
  ...
  const content = lessonData[params.topic] || defaultLessonContent
  ...
  // Get the specific data for the current step
  const getStepData = () => {
    switch (step) {
      case 0: return { words: content.vocabulary }
      case 1: return { data: content.listening }
      case 2: return { questions: content.practice }
      case 4: return { questions: content.finalTest }
      default: return {}
    }
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="glass rounded-2xl p-6 border border-white/08 overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              <StepComponent onComplete={handleComplete} {...getStepData()} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
